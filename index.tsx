...
const DX_MAP = [
  ...
  [34,7,28,1,33,25,0,24,19], [13,34,7,21,28,1,30,33,25,0,14,24,31], [31,19], [34,7,28,1,33,25,19], [7,21,28,1,33,24,31,19], 
...
const App: React.FC = () => {
  const [selectedCategoryId, setSelectedCategoryId] = useState<number>(() => {
    try {
      ...
      return saved ? JSON.parse(saved).cat : 0;
    } catch { return 0; }
  });
  const [selectedInventorIds, setSelectedInventorIds] = useState<number[]>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      return saved ? JSON.parse(saved).invs : [];
    } catch { return []; }
  });
  const [hasSpecialItem, setHasSpecialItem] = useState<boolean>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      return saved ? JSON.parse(saved).spec : false;
    } catch { return false; }
  });
  const [selectedItemId, setSelectedItemId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      return saved ? JSON.parse(saved).item : null;
    } catch { return null; }
  });
  ...
  const handleCategorySwitch = (id: number) => {
  ...
  const suggestions = useMemo(() => {
    ...
    const targetItem = ITEMS[selectedItemId];
    
    return availableInventors
      .filter(inv => !selectedInventorIds.includes(inv.id))
      .map(inv => {
        let virtualTeam = [...selectedInventorIds];
        if (virtualTeam.length < 3) {
          virtualTeam.push(inv.id);
        } else {
          const lowestSkillId = team.length > 0 ? team.reduce((l, c) => c.skills[selectedCategoryId] < l.skills[selectedCategoryId] ? c : l, team[0]).id : inv.id;
          virtualTeam = virtualTeam.map(id => id === lowestSkillId ? inv.id : id);
        }

        const virtualInventors = virtualTeam.map(id => INVENTORS.find(i => i.id === id)!);
        const virtualSkill = virtualInventors.reduce((acc, v) => acc + v.skills[selectedCategoryId], hasSpecialItem ? 20 : 0);
        const virtualCostMod = virtualInventors.reduce((acc, v) => acc + v.costMod, 0);
        
        const baseSkill = Math.max(stats.totalSkill, 1);
        const skillBoostPct = Math.round(((virtualSkill - baseSkill) / baseSkill) * 100);
        
        const virtualTicks = getTicksHelper(targetItem.baseCost, virtualCostMod);
        const otherItems = ITEMS.filter(i => i.categoryId === selectedCategoryId && i.id !== selectedItemId);
        
        let collisionCount = 0;
        for (const other of otherItems) {
            const otherTicks = getTicksHelper(other.baseCost, virtualCostMod);
            const hasOverlap = otherTicks.some(ot => virtualTicks.includes(ot));
            if (hasOverlap) collisionCount++;
        }

        const collisionPenalty = collisionCount * 30;
        const score = skillBoostPct - collisionPenalty + (Math.abs(Math.min(0, inv.costMod)) / 2);

        return { inv, boostPct: skillBoostPct, cost: inv.costMod, collisions: collisionCount, score };
      })
      .filter(s => s.score > -100)
      .sort((a, b) => b.score - a.score)
      .slice(0, 3);
  }, [selectedItemId, selectedCategoryId, selectedInventorIds, availableInventors, team, stats.totalSkill, hasSpecialItem]);
  ...
  const targetTicks = useMemo(() => {
    if (selectedItemId === null) return [];
    return getTicksHelper(ITEMS[selectedItemId].baseCost, stats.totalCostMod);
  }, [selectedItemId, stats.totalCostMod]);

  const overlaps = useMemo(() => {
    if (selectedItemId === null || targetTicks.length === 0) return [];
    return ITEMS.filter(item => 
      item.categoryId === selectedCategoryId && item.id !== selectedItemId && item.allowedInventors.some(ai => selectedInventorIds.includes(ai))
    )
    .map(item => {
      const itemTicks = getTicksHelper(item.baseCost, stats.totalCostMod);
  ...
  return (
    <div className="max-w-5xl mx-auto p-2 sm:p-4 text-slate-300 text-xs selection:bg-blue-500/30 no-scrollbar">
      <header className="mb-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div className="flex items-center gap-4">
          <h1 className="font-black text-white text-base tracking-tighter flex items-center gap-2">
            <div className="w-2.5 h-6 bg-blue-600 rounded-full shadow-[0_0_12px_rgba(37,99,235,0.6)]" /> SO3 <span className="text-slate-500 font-medium">IC·LAB</span>
          </h1>
          <button onClick={handleReset} className="px-3 py-1 bg-slate-800/50 hover:bg-rose-900/20 text-slate-500 hover:text-rose-400 border border-slate-700/50 rounded text-[9px] font-black uppercase tracking-widest transition-all">Reset Session</button>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar max-w-full">
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => handleCategorySwitch(c.id)} 
        ...
        <div className="md:col-span-4 space-y-3">
          <div className="bg-slate-900/50 rounded-lg border border-slate-800 p-2">
            <div className="flex justify-between items-center mb-2 px-1 text-slate-500 font-bold uppercase tracking-widest text-[10px]">
              <span>Inventor Registry</span>
              <span className="text-blue-500 font-mono">{selectedInventorIds.length}/3</span>
            </div>
            <div className="grid grid-cols-2 gap-1.5 max-h-[400px] overflow-y-auto pr-1 no-scrollbar">
        ...
        <div className="md:col-span-4 space-y-3">
          <div className="bg-slate-900/50 rounded-lg border border-slate-800 p-3">
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3 bg-slate-800/40 rounded-md border border-slate-700/30 text-center">
                <div className="text-[10px] uppercase text-slate-500 font-black mb-1">Team Skill</div>
          ...
          {suggestions.length > 0 && (
            <div className="bg-blue-600/5 rounded-lg border border-blue-500/20 p-3 animate-in slide-in-from-bottom-2">
              <div className="text-[10px] font-black uppercase text-blue-400/80 mb-2.5 flex items-center justify-between">
                ...
                <span className="text-[8px] text-slate-500 opacity-60">No Collisions Found</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {suggestions.map(s => (
                  <button key={s.inv.id} onClick={() => optimize(s.inv.id)} className="flex-1 min-w-[120px] flex items-center gap-2 px-3 py-2 bg-slate-900/80 border border-slate-700 rounded-md hover:border-blue-500 transition-all group relative">
                    <div className="flex flex-col items-start w-full">
                      <div className="flex items-center justify-between w-full gap-1.5">
                        <span className="font-bold text-slate-200 group-hover:text-blue-300 transition-colors truncate">{s.inv.name}</span>
                        {s.collisions === 0 && (
                            <svg className="w-3 h-3 text-emerald-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                        )}
                      </div>
                      <div className="flex justify-between w-full mt-0.5">
                         <span className={`text-[9px] font-black ${s.boostPct >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{s.boostPct >= 0 ? '+' : ''}{s.boostPct}% Skill</span>
                         <span className={`text-[9px] font-black ${s.collisions === 0 ? 'text-emerald-400' : 'text-slate-500'}`}>{s.collisions} Overlaps</span>
                      </div>
                    </div>
                  </button>
                ))}
          ...
          {selectedItemId !== null ? (
            <div className="bg-slate-900/50 rounded-lg border border-slate-800 p-3 animate-in fade-in zoom-in-95 duration-300">
              <div className="flex justify-between items-center mb-3 px-1">
                <span className="font-black uppercase text-slate-500 tracking-tighter">Calibration Ticks</span>
              ...
              {overlaps.length > 0 && (
                <div className="border-t border-slate-800 pt-4 space-y-3">
                  <div className="text-[10px] font-black uppercase text-rose-400/80 flex items-center gap-2">
                    <div className="w-2 h-4 bg-rose-500 rounded-full shadow-[0_0_8px_rgba(244,63,94,0.4)]" /> Collision Map
                  </div>
                    ...
                    {overlaps.map(o => (
                      <div key={o.item.id} className="text-[10px] bg-slate-950/50 p-3 rounded-md border border-slate-800/50 flex flex-col gap-1 group/coll">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          <span className="text-slate-500 text-[9px] font-bold uppercase tracking-tighter mr-1">Conflicts:</span>
      ...
      <footer className="mt-8 pt-4 border-t border-slate-800/50 flex justify-between text-[9px] text-slate-700 font-black uppercase tracking-widest">
        <span>Star Ocean 3: Till the End of Time</span>
        <span>PRO CALC DASHBOARD V3.4</span>
      </footer>
    </div>
  );
};
